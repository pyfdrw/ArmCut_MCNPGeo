...
const int ORGANNUM = 142;
float ORGANSDESTINY[ORGANNUM] = { 0 }; // store Density data
int ORGANMATERIAL[151] = { 0 };        // store Material data
...
int outputGeo(McnpFillStruct mcnpgeo999)
{
    ...
    *voxelidtmp = '\0';
    ...
    for (int i = 0; i < 200; i++)
    {
        if ('\0' != *(namelist + i * 80))
        {
            fprintf(outputfile, "%-5d %-5d %-5d %s %-4d %s %-.4f %s %-d", \
...
void lenceGen(char* inputpath, unsigned int lence, unsigned int bulb, McnpFillStruct& mcnpgeo999)
{
    ...
    *objfilepath = '\0';
    ...
    else
    {
        ...
        float lenceradius;
        lenceradius = pow(fabs(calGeoVol(calGeoCenter(objgeo), objgeo)) / 4.0 * 3.0 / 3.14159, 1.0 / 3.0);
        Point lencecenter;
        ...
        for (int i = 0; i < mcnpgeo999.voxelcount; i++)
        {
            if (bulb == *(mcnpgeo999.element + i))
            {
                ...
                if ((coordX - lencecenter.getcoorX()) * (coordX - lencecenter.getcoorX()) +
                    ...
                    (coordZ - lencecenter.getcoorZ()) * (coordZ - lencecenter.getcoorZ()) <= lenceradius * lenceradius)
                {
...
void calBox(McnpFillStruct mcnpgeoA, int organid)
{
        ...
        for (int i = 0; i < mcnpgeo999.voxelcount; i++)
        {
            if (lenseid == *(mcnpgeo999.element + i))
            {
                ...
                if (*(mcnpgeo999.element + i - 1) == lenseid + 1 || *(mcnpgeo999.element + i - 1) == 150 || \
                    *(mcnpgeo999.element + i + 1) == lenseid + 1 || *(mcnpgeo999.element + i + 1) == 150)
                {
...
int armCut(McnpFillStruct &mcnpgeoA, float xmin , float xmax, float ymin , float ymax, float zmin , float zmax)
{
    ...
    {
        Coordinate3D XYZtmp;
        XYZtmp = convertSequenceToXYZ(i, mcnpgeoA);
        XYZtmp.x = XYZtmp.x * mcnpgeoA.voxelsizex;
        XYZtmp.y = XYZtmp.y * mcnpgeoA.voxelsizey;
        XYZtmp.z = XYZtmp.z * mcnpgeoA.voxelsizez;
        if (XYZtmp.x >= xmin && XYZtmp.x <= xmax &&
            XYZtmp.y >= ymin && XYZtmp.y <= ymax &&
            XYZtmp.z >= zmin && XYZtmp.z <= zmax)
        {
            *(mcnpgeoA.element + i) = 150;
        }
    }
